...
var PredefinedRanges = (function (_Component) {
  ...
  _createClass(PredefinedRanges, [{
    ...
    value: function handleSelect(name, event) {
      ...
      if (this.props.customOnRangeChange) {
        this.props.customOnRangeChange(name);
      } else {
        this.props.onSelect({
    ...
    value: function renderRangeList(classes) {
      ...
      var includeContainedRanges = _props.includeContainedRanges;
      var styles = this.styles;

      return Object.keys(ranges).map((function (name) {
        var isStart = (0, _utilsParseInputJs2['default'])(ranges[name].startDate).isSame(range.startDate) && (0, _utilsParseInputJs2['default'])(ranges[name].endDate).isBefore(range.endDate);
        var isEnd = (0, _utilsParseInputJs2['default'])(ranges[name].startDate).isAfter(range.startDate) && (0, _utilsParseInputJs2['default'])(ranges[name].endDate).isSame(range.endDate)
        var isContained = (0, _utilsParseInputJs2['default'])(ranges[name].startDate).isAfter(range.startDate) && (0, _utilsParseInputJs2['default'])(ranges[name].endDate).isBefore(range.endDate);
        var isExactMatch = (0, _utilsParseInputJs2['default'])(ranges[name].startDate).isSame(range.startDate) && (0, _utilsParseInputJs2['default'])(ranges[name].endDate).isSame(range.endDate);
        var active = includeContainedRanges ? (isContained || isExactMatch || isStart || isEnd) : isExactMatch;
...
PredefinedRanges.propTypes = {
  ranges: _PropTypes.object.isRequired,
  onlyClasses: _PropTypes.bool,
  includeContainedRanges: _PropTypes.bool,
